...
string CacheIP = "gtroyal.ddns.net";
...
inline void SendConsole(const string text, const string type)
{
	...
	const auto Min = localTime->tm_min;
	const auto Sec = localTime->tm_sec;
	if (type != "CHAT" && type != "BAN" && type != "LOGS" && type != "LOGS2" && type != "ERROR")
	{
		cout << "[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + " " + type + "]: " << text << endl;
	}
	if (type == "BAN" && text.find("@everyone") == string::npos && text.find("https") == string::npos && text.find("discord.gg") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php ban.php " + superrealtext + "";
		const auto c = themessage.c_str();
		system(c);
	}
	if (type == "ERROR" && text.find("https") == string::npos && text.find("discord.gg") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php error.php " + superrealtext + "";
		const auto c = themessage.c_str();
		system(c);
	}
	if (type == "LOGS" && text.find("https") == string::npos && text.find("discord.gg") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php logs.php " + superrealtext + "";
		const auto c = themessage.c_str();
		system(c);
	}
	if (type == "LOGS2" && text.find("https") == string::npos && text.find("discord.gg") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php logs2.php " + superrealtext + "";
		const auto c = themessage.c_str();
		system(c);
	}
	if (type == "REFUND" && text.find("https") == string::npos && text.find("discord.gg") == string::npos)
	{
		const auto superrealtext = space2underscore(text);
		const auto themessage = "start php refund.php " + superrealtext + "";
		const auto c = themessage.c_str();
		system(c);
	}
}
