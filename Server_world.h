...
struct WorldInfo
{
	...
	bool isjammed = false;
	bool ispunch = false;
	long long int lastjoin = 0;
	long long int lastmachine = 0;
	long long int lastsave = 0;
	long long int lastsave2 = 0;
	int ownerID = 0;
...
inline void GenerateGrowaloneLikeWorld1(WorldInfo* world, const int width, const int height, const int dirtType = 2, const int lavaType = 4, const int mainDoorType = 6, const int bedrockType = 8, const int rockType = 10, const int caveBackgroundType = 14, const int wmachineid = 0, const int deco1Id = 0, const int deco2Id = 0, const int id = 0)
{
	//SendConsole("Generating new world (GenerateGrowaloneLikeWorld) details: width(" + to_string(width) + ") height(" + to_string(height) + ") dirtType(" + to_string(dirtType) + ") lavaType(" + to_string(lavaType) + ") mainDoorType(" + to_string(mainDoorType) + ") bedrockType(" + to_string(bedrockType) + ") rockType(" + to_string(rockType) + ") caveBackgroundType(" + to_string(caveBackgroundType) + ") wmachineid(" + to_string(wmachineid) + ") deco1Id(" + to_string(deco1Id) + ") deco2Id(" + to_string(deco2Id) + ") id(" + to_string(id) + ")", "INFO");
	world->weather = wmachineid;
	for (auto i = 0; i < world->width * world->height; i++)
...
inline void WorldDB::flush(WorldInfo info)
{
	...
	j["bid"] = info.bgID;
	j["ispunch"] = info.ispunch;
	j["isjammed"] = info.isjammed;
	j["started"] = info.isstarted;
	j["hwr"] = info.hw_red;
	j["hwg"] = info.hw_green;
	j["hwb"] = info.hw_blue;
	j["isPublic"] = info.isPublic;
	j["event"] = info.enableevent;
	j["accessworld"] = info.accessed;
	j["gem"] = info.hgem;
	j["dc"] = info.droppedItems.size();
	j["nuked"] = info.isNuked;
	j["zombie"] = info.zombie;
	j["toto"] = info.breakid;
	j["toto1"] = info.lastmachine;
	j["toto2"] = info.technogems;
	...
	for (auto i = 0; i < info.droppedItems.size(); i++)
	{
		...
		droppedJ["madeby"] = info.droppedItems[i].madeby;
	...
	for (auto i = 0; i < square; i++)
	{
		...
		tile["flipped"] = info.items[i].flipped;
...
AWorld WorldDB::get2(string name) {
	...
	try {
		...
		if (ifs.is_open()) {
			...
			try {
				info.ispunch = j["ispunch"].get<bool>();
			}
			catch (nlohmann::json::exception ex) {
				info.ispunch = false;
			}
			try {
				info.isstarted = j["started"].get<bool>();
			}
			catch (nlohmann::json::exception ex) {
			...
			try {
				info.hgem = j["gem"].get<int>();
			}
			catch (nlohmann::json::exception ex) {
				info.hgem = 0;
			}
			info.stuff_invert = j["si"].get<bool>();
			info.enableevent = j["event"].get<bool>();
			info.zombie = j["zombie"].get<bool>();
			...
			try {
				info.breakid = j["toto"].get<int>();
			}
			catch (nlohmann::json::exception ex) {
				info.breakid = 0;
			}
			try {
				info.lastmachine = j["toto1"].get<long long int>();
			}
			catch (nlohmann::json::exception ex) {
				info.lastmachine = 0;
			}
			try {
				info.technogems = j["toto2"].get<int>();
			}
			catch (nlohmann::json::exception ex) {
				info.technogems = 0;
			}
			...
			for (auto i = 0; i < info.droppedCount; i++)
			{
				...
				di.madeby = droppedobjects[i]["madeby"].get<string>();
			...
			for (auto i = 0; i < square; i++)
			{
				...
				info.items[i].flipped = tiles[i]["flipped"].get<bool>();
				info.items[i].activated = tiles[i]["activated"].get<bool>();
				info.items[i].dshelf1 = tiles[i]["dshelf1"].get<int>();
				info.items[i].dshelf4 = tiles[i]["dshelf4"].get<int>();
				info.items[i].dshelf3 = tiles[i]["dshelf3"].get<int>();
				info.items[i].dshelf2 = tiles[i]["dshelf2"].get<int>();
			}
		...
		SendConsole("Critical error Aworld Worlddb Get2 (" + name + ")", "ERROR");
...
inline BOOL WINAPI HandlerRoutine(DWORD dwCtrlType)
{
	saveAllWorlds();
	return FALSE;
}
...
inline WorldInfo* getPlyersWorld(ENetPeer* peer)
{
	if (serverIsFrozen == false && static_cast<PlayerInfo*>(peer->data)->currentWorld != "EXIT")
	{
		if (peer != nullptr)
		{
			...
			{
				SendConsole("Failed returning value (getPlyersWorld)", "ERROR");
			}
		}
	}
	return nullptr;
...
inline void SaveMyWorld(ENetPeer* peer) {
	try {
		auto world = getPlyersWorld(peer);
		if (static_cast<PlayerInfo*>(peer->data)->currentWorld == "EXIT") return;
		if (peer->state != ENET_PEER_STATE_CONNECTED) return;
		std::ofstream o("worlds/_" + world->name + ".json");
		if (!o.is_open())
		{
		}
		json j;
		...
		j["owner"] = world->owner;
		j["dname"] = world->ownerDisplayName;
		j["isPublic"] = world->isPublic;
		j["weather"] = world->weather;
		j["sid"] = world->stuffID;
		j["sil"] = world->silence;
		j["rbg"] = world->rainbows;
		j["ispunch"] = world->ispunch;
		j["isjammed"] = world->isjammed;
		j["sg"] = world->stuff_gravity;
		j["si"] = world->stuff_invert;
		j["ss"] = world->stuff_spin;
		j["bid"] = world->bgID;
		j["hwr"] = world->hw_red;
		j["hwg"] = world->hw_green;
		j["hwb"] = world->hw_blue;
		j["isPublic"] = world->isPublic;
		j["event"] = world->enableevent;
		j["accessworld"] = world->accessed;
		j["dc"] = world->droppedItems.size();
		j["nuked"] = world->isNuked;
		j["zombie"] = world->zombie;
		j["toto"] = world->breakid;
		j["toto1"] = world->lastmachine;
		j["toto2"] = world->technogems;
		auto tiles = json::array();
		auto droppedarr = json::array();
		const auto square = world->width * world->height;
		for (auto i = 0; i < world->droppedItems.size(); i++)
		{
			json droppedJ;
			droppedJ["c"] = static_cast<BYTE>(world->droppedItems[i].count);
			droppedJ["id"] = static_cast<short>(world->droppedItems[i].id);
			droppedJ["x"] = world->droppedItems[i].x;
			droppedJ["y"] = world->droppedItems[i].y;
			droppedJ["droppedby"] = world->droppedItems[i].droppedby;
			droppedJ["uid"] = world->droppedItems[i].uid;
			droppedJ["madeby"] = world->droppedItems[i].madeby;
			droppedarr.push_back(droppedJ);
		}
		j["dropped"] = droppedarr;
		for (auto i = 0; i < square; i++)
		{
			json tile;
			tile["fg"] = world->items[i].foreground;
			tile["bg"] = world->items[i].background;
			tile["s"] = world->items[i].sign;
			tile["r"] = world->items[i].flipped;
			tile["int"] = world->items[i].intdata;
			tile["water"] = world->items[i].water;
			tile["fire"] = world->items[i].fire;
			tile["glue"] = world->items[i].glue;
			tile["flipped"] = world->items[i].flipped;
			tile["activated"] = world->items[i].activated;
			tile["dshelf1"] = world->items[i].dshelf1;
			tile["dshelf2"] = world->items[i].dshelf2;
			tile["dshelf3"] = world->items[i].dshelf3;
			tile["dshelf4"] = world->items[i].dshelf4;
			tiles.push_back(tile);
		}
		j["tiles"] = tiles;
		o << j << std::endl;

		world->isworldsave = false;
	}
	catch (std::runtime_error&)
	{
		SendConsole("SaveMyWorld error (runtime)", "ERROR");
		return;
	}
	catch (...)
	{
		SendConsole("SaveMyWorld error (...)", "ERROR");
		return;
	}
}
inline void Save()
{
	while (true)
	{
		try {
			...
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
				Player::OnConsoleMessage(currentPeer, "`w[(megaphone)] `$Worlds are saving, /find price was getting randomized.");
			}
			serverIsFrozen = true;
			saveAllWorlds();
			serverIsFrozen = false;
			...
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
				Player::OnConsoleMessage(currentPeer, "`w[(megaphone)] `$Worlds are saved, /find price was already randomized.");
			}
		}
		catch (std::exception&)
		{
			SendConsole("save system Critical error", "ERROR");
		}
		catch (std::runtime_error&)
		{
			SendConsole("save system Critical error", "ERROR");
	...
	while (true)
	{
		try {
			...
			for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
				auto valgem = 500;
				ifstream ifs("gemdb/" + static_cast<PlayerInfo*>(currentPeer->data)->rawName + ".txt");
				...
				Player::OnConsoleMessage(currentPeer, "`w[(gems)] `$Bonus : `2+500 `$gems!");
			}
		}
		catch (std::exception&)
		{
	...
	while (true)
	{
		Sleep(600000);
		{
			...
			for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
				if (static_cast<PlayerInfo*>(currentPeer->data)->currentWorld == "EXIT") continue;
				Player::OnConsoleMessage(currentPeer, "`w[(gems)] `5** `$Helltopia `cMath Event `o: `$What is the answer of `5" + to_string(math1) + " `$+ `5" + to_string(math2) + "`$? `w[(gems)] `5**");
				Player::OnConsoleMessage(currentPeer, "`w[(gems)] `5** `$Prize is : `c `5" + to_string(prize) + " (gems) `o(`$/a <answer> `oto answer) `w[(gems)] `5**");
			}
		}
		Sleep(600000);
		int val = rand() % 200;
		...
		for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
		{
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
			if (static_cast<PlayerInfo*>(currentPeer->data)->currentWorld == "EXIT") continue;
			Player::PlayAudio(currentPeer, "audio/thunderclap.wav", 0);
			Player::OnConsoleMessage(currentPeer, "`w[(gems)] `5** `$Helltopia `cMath Event `o: `$What is the answer of `5" + to_string(math1) + " `$x `5" + to_string(math2) + "`$? `w[(gems)] `5**");
			Player::OnConsoleMessage(currentPeer, "`w[(gems)] `5** `$Prize is : `c `5" + to_string(prize) + " (gems) `o(`$/a <answer> `oto answer) `w[(gems)] `5**");
		}
		Sleep(600000);
		{
	...
	if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
	{
		if (static_cast<PlayerInfo*>(peer->data)->rawName == getPlyersWorld(peer)->owner)
		{
			if (isMod(peer))
			{
				...
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2@Mod " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
			}
		}
		else
...
{
	if (find(world->accessed.begin(), world->accessed.end(), static_cast<PlayerInfo*>(peer->data)->rawName) != world->accessed.end()) return true;
}
...
inline void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
{
	...
	for (auto i = 0; i < square; i++)
	{
		...
		else if (getItemDef(tile).blockType == BlockTypes::SIGN || tile == 1420 || tile == 6214 || tile == 9202 || tile == 10072 || tile == 10074 || tile == 10076 || tile == 10078)
		{
			auto type = 0x00000000;
			if (worldInfo->items[i].activated)
				type |= 0x00200000;
	...
	if (xSize == 100 && ySize == 60)
	{
		for (auto i = 0; i < xSize; i++)
		{
			for (auto j = 0; j < ySize; j++)
			{
				...
				if (ismannequin)
				{
					if (worldInfo->items[squaresign].foreground == 1420 || worldInfo->items[squaresign].foreground == 6214 || worldInfo->items[squaresign].foreground == 9202 || worldInfo->items[squaresign].foreground == 10072 || worldInfo->items[squaresign].foreground == 10074 || worldInfo->items[squaresign].foreground == 10076 || worldInfo->items[squaresign].foreground == 10078)
					{
						json j;
...
inline void TangaTest(ENetPeer* peer, bool tanga, int bubu)
{
	
	while (true)
	{
		Sleep(1000); /*every 10 minutes event happens*/
		{
			if (serverIsFrozen) return;
			auto world = getPlyersWorld(peer);
			if (world->ismachine == false) return;
			if (!world) return;
			if (static_cast<PlayerInfo*>(peer->data)->currentWorld == "EXIT") return;
			...
			if (xSize == 100 && ySize == 60)
			{
				for (auto i = 0; i < xSize; i++)
				{
					...
					{
						auto squaresign = i + (j * 100);
						if (world->items[squaresign].foreground == bubu)
						{
							auto x = squaresign % world->width;
							auto y = squaresign / world->width;
							world->items[squaresign].foreground = 0;
							PlayerMoving data3;
							data3.packetType = 0x3;
							data3.characterState = 0x0;
							data3.x = x;
			...
			/*for (auto i = 0; i < world->width * world->height; i++)
			{
				if (getItemDef(bubu).blockType == BlockTypes::FOREGROUND) {
					if (world->items[i].foreground == bubu) {
						if (bubu == 5136) {
							auto valgem = rand() % 450;
							world->technogems = world->technogems + valgem;
						}
						else if (bubu == 5138) {
							auto valgem = rand() % 650;
							world->technogems = world->technogems + valgem;
						}
						else if (bubu == 5156) {
							auto valgem = rand() % 1600;
							world->technogems = world->technogems + valgem;
						}
						else if (bubu == 5158) {
							auto valgem = rand() % 2300;
							world->technogems = world->technogems + valgem;
						}
						else if (bubu == 5140) {
							auto valgem = rand() % 900;
							world->technogems = world->technogems + valgem;
						}
						...
						for (currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
						{
							...
							if (isHere(peer, currentPeer))
							{
								...
								raw[3] = 0;
								SendPacketRaw(4, raw, 56, nullptr, currentPeer, ENET_PACKET_FLAG_RELIABLE);
								Player::OnParticleEffect(currentPeer, 229, xx * 32, yy * 32, 0);
								Player::PlayAudio(currentPeer, "audio/metal_destroy.wav", 0);
							}
						}
					}
					world->ismachine = false;
				}
				else {
					world->breakid = 0;
		...
		else
		{
			...
			if (act == "WINTER") {
				if (!isDev(peer)) {
					...
					SearchInventoryItem(peer, 5142, 1, iscontainss);
					if (iscontainss) {
						Player::OnTextOverlay(peer, "You completed the quest already!");
						Player::OnFailedToEnterWorld(peer);
						return;
					}
				}
			...
			if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
			{
				...
				if (static_cast<PlayerInfo*>(peer->data)->timeTogetToken <= GetCurrentTimeInternalSeconds())
				{
					...
					else
					{
						...
						SaveItemMoreTimes(9426, 1, peer, checkSuccess, static_cast<PlayerInfo*>(peer->data)->rawName + " from system");
						if (!checkSuccess)
						{
			...
			if (info.enableevent != true || info.zombie != false || info.ispunch != false || info.isjammed != false) {
				string jammed;
				string isevent;
				string ispunch;
				string zombie;
				...
				Player::OnConsoleMessage(peer, "`$Jammers : `w" + jammed + "" + isevent + "" + ispunch + "" + zombie + "");
			}
			SendInventory(peer, static_cast<PlayerInfo*>(peer->data)->inventory);
			auto x = 3040;
			auto y = 736;
...
inline void playerRespawn(ENetPeer* peer, const bool isDeadByTile)
{
	try {
		...
		SendConsole("Critical error void playerrespawn : rawName(" + static_cast<PlayerInfo*>(peer->data)->rawName + ")", "ERROR");
		enet_peer_disconnect_now(peer, 0);
		return;
	}
	catch (std::runtime_error&) {
...
inline void SendChat(ENetPeer* peer, const int netID, string message)
{
	try {
		...
		using namespace std::chrono;
		if (((PlayerInfo*)(peer->data))->lastCHAT + 500 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
		{
			((PlayerInfo*)(peer->data))->lastCHAT = (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count();
		}
		else {
			((PlayerInfo*)(peer->data))->puta = ((PlayerInfo*)(peer->data))->puta + 1;
			if (((PlayerInfo*)(peer->data))->puta == 3)
			{
				((PlayerInfo*)(peer->data))->lastInfoAboutPlayer = ((PlayerInfo*)(peer->data))->rawName;
				BanTime(peer, 5, true, "Failed 3x on Anti-Spam System");
			}
			Player::OnConsoleMessage(peer, "`4WARNING : `$Take it easy with messaging! or else, we will ban you!");
			return;
		}
		string ccode;
		...
		else if (((PlayerInfo*)(peer->data))->adminLevel == 4) {
			ccode = "#";
		}
		else {
			ccode = "w";
		...
		SendConsole("Critical error void sendchat : rawName(" + static_cast<PlayerInfo*>(peer->data)->rawName + ")", "ERROR");
		enet_peer_disconnect_now(peer, 0);
		return;
	}
	catch (std::runtime_error&) {
...
inline void SendTake(ENetPeer* peer, const int netID, const int x, const int y, const int item)
{
	try {
		...
		SendConsole("Critical error void sendinventory : rawName(" + static_cast<PlayerInfo*>(peer->data)->rawName + ")", "ERROR");
		enet_peer_disconnect_now(peer, 0);
		return;
	}
	catch (std::runtime_error&) {
...
inline void SendFarmableDrop(ENetPeer* peer, int chance)
{
	try
	{
		...
		ifstream ifs("gemdb/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
		const string content((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
		Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "`wYou got `5" + std::to_string(valgem) + " `wgems`w!", 0, false);
		const auto gembux = atoi(content.c_str());
		const auto fingembux = gembux + valgem;
		ofstream myfile;
...
inline void SendMergeDialog(ENetPeer* peer, int item1, int item2, int item3, string extralabel)
{
	...
	if (item1 == 0) item1 = 9412;
	if (item2 == 0) item2 = 9412;
	if (item3 == 0) item3 = 6864;
	if (item1 != 9412 && item2 != 9412 && item3 == 6864)
	{
	...
	string item3name = "Merging Result";
	if (item1 != 9412) item1name = getItemDef(item1).name;
	if (item2 != 9412) item2name = getItemDef(item2).name;
...
*/
inline void ngekngek(ENetPeer* peer, int x, int y) {
	...
	while (true)
	{
		Sleep(3000);
		{
			...
			auto world = getPlyersWorld(peer);
			if (!world) return;
			if (world->isstarted == false) return;
...
inline void scan_owner(ENetPeer* peer, const string oname)
{
	...
	Player::OnTextOverlay(peer, "This feature coming soon!");
	/*string path = "worlds/";
	int fun = 0;
	string who = "";
	int worlds = 0;
...
inline void ngentot(ENetPeer* peer)
{
	...
	try
	{
		...
		for (auto i = 0; i < 200; i++)
		{
			if (i < invsize)
			{
				...
				jj["madeby"] = static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).madeBy;
				jjall.push_back(jj);
			}
			else
			{
	...
	{
		SendConsole("Save Inventory Disconnect (exception) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + "", "ERROR");
		enet_peer_disconnect_now(peer, 0);
	}
	catch (std::runtime_error&)
	{
		SendConsole("Save Inventory Disconnect (runtime) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + "", "ERROR");
		enet_peer_disconnect_now(peer, 0);
	}
	catch (...)
	{
		SendConsole("Save Inventory Disconnect (...) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + "", "ERROR");
		enet_peer_disconnect_now(peer, 0);
	}
}
...
inline void TradeMainMenu(ENetPeer* peer)
{
		...
		++currentPeer)
	{
		...
		if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == PlayerDB::getProperName(user))
		{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putMySellingItem_1", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putMySellingItem_2", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putMySellingItem_3", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putMySellingItem_4", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putHisSellingItem_1", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putHisSellingItem_2", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putHisSellingItem_3", false);
			}
			else
			{
			...
			{
				tradedialog.addStaticBlueFrameWithIdCountText("9412", "0", "empty", "putHisSellingItem_4", false);
			}
			else
			{
